...
#SERVERPORT="8090"
SERVERIP="172.31.41.1"
SERVERPORT="8090"
#---------------------------   end  -------------------------

TYPE=$1
INTERFACE="enX0"  # Update this to your actual interface name
...
exec_ifup(){
    for i in "${!IPS[@]}"; do
        echo ifconfig $INTERFACE:$i ${IPS[$i]} netmask $NETMASK up
        ifconfig $INTERFACE:$i ${IPS[$i]} netmask $NETMASK up
    done
}

exec_ifdown(){
    for i in "${!IPS[@]}"; do
        echo ifconfig $INTERFACE:$i down
